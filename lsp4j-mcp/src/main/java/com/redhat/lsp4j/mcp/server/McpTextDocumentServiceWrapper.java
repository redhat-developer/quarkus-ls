...
/**
 * Wrapper for TextDocumentService that intercepts didOpen/didClose to track
 * which files are opened in the IDE.
 *
 * All other methods are delegated to the wrapped service.
 */
public class McpTextDocumentServiceWrapper implements TextDocumentService {

	private final TextDocumentService delegate;
	private final McpCache cache;

	public McpTextDocumentServiceWrapper(TextDocumentService delegate, McpCache cache) {
	...
	public void didOpen(DidOpenTextDocumentParams params) {
		...
		cache.onDidOpen(params.getTextDocument().getUri());

		// Forward to the real language server
		delegate.didOpen(params);
	}

	@Override
	public void didClose(DidCloseTextDocumentParams params) {
		// Track that this file is now closed
		cache.onDidClose(params.getTextDocument().getUri());

		// Forward to the real language server
		delegate.didClose(params);
	}

	// All methods below are simple delegation - no interception needed
