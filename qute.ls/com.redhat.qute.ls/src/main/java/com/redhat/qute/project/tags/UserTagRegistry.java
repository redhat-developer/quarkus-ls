...
public class UserTagRegistry {
	 ...
	 */
	public void updateUsages(String uri, Map<String, List<Parameter>> usages, Set<String> oldTagNames) {

		for (Map.Entry<String, List<Parameter>> entry : usages.entrySet()) {
			String tagName = entry.getKey();
			UserTagUsages tagUsages = getOrCreateUsages(tagName);
			tagUsages.updateUsages(uri, entry.getValue());
		}
		...
		for (String tagName : oldTagNames) {
			...
			if (tagUsages != null) {
				tagUsages.updateUsages(uri, List.of());
			}
		}
	}
